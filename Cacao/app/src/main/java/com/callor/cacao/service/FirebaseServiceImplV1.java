 ...
 * Firebase에서 Android 모든 App에 신호를 보내고 그 신호를 Event로 받아서 처리하는 클래스를 작성하기 위한 interface
 */
public class FirebaseServiceImplV1 implements ChildEventListener {
     ...
     *
     * RecyclerView와 연동하여 데이터를 화면에 그리기 위한 중간 매개체(연결도구)
     *
     * Adapter를 MainActivity로부터 전달받아서 firebase Database에 데이터가 추가되면 데이터를 가져와서 화면에 그리기 위한 코드 작성
     */
    ...
    /*
     * CUD 중에서 새로운 데이터가 추가되는 ( Insert OR Create ) Event가 발생하면 실행되는 method
     *
     * 이 method에서 데이터가 추가되었다는 신호를 받으면 데이터를 가져와서 Adapter의 ChattList에 데이터를 추가한다
